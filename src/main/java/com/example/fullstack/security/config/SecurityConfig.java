...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
               ...
               // .authorizeHttpRequests(auth -> auth.anyRequest().permitAll())
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/menuItem/get/category/**","/auth/**","/api/menuItem/get/name/size","/api/menuItem/items").permitAll() // allow unauthenticated access to login/signup
...
//                .maxSessionsPreventsLogin(false))
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        CorsConfiguration configuration = new CorsConfiguration();
        configuration.setAllowedOrigins(List.of("http://localhost:5173","https://pizza-point-k53k.vercel.app")); // Allow frontend
